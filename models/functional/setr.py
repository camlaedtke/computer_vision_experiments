...
class SETR(tf.keras.models.Model):
    def __init__(self, 
                 image_size, 
                 patch_size, 
                 num_layers, 
        ...
        repr_size = image_size // patch_size
        
        self.input_layers = tf.keras.Sequential([
            Conv2D(
                filters=hidden_size, 
            ...
            Reshape((repr_size * repr_size, hidden_size)),
            ClassToken(name="class_token"),
            AddPositionEmbs(name="Transformer/posembed_input")
        ])
        
        self.extract_token = Lambda(lambda x: x[:, 1:], name="ExtractToken")
        self.final_layer_norm = LayerNormalization(epsilon=1e-6, name="Transformer/encoder_norm_decode")
        
        self.aux_heads = [self.get_aux_head(self.aux_layers[i]) for i in self.aux_layers]
        
        self.decode_head = tf.keras.Sequential([
            Reshape(target_shape=(int(self.image_size//16), int(self.image_size//16), self.hidden_size)),
            
            UpSampling2D(size=(2,2), interpolation='bilinear', name="Decode_upsample_1"),
            Conv2D(self.hidden_size, kernel_size=(3,3), strides=(1,1), padding='same', name="Decode_conv_1"),
            GroupNormalization(groups=32, name="Decode_gn_1"),
            Activation("relu", name="Decode_relu_1"),
            
            UpSampling2D(size=(2,2), interpolation='bilinear', name="Decode_upsample_2"),
            Conv2D(256, kernel_size=(3,3), strides=(1,1), padding='same', name="Decode_conv_2"),
            GroupNormalization(groups=32, name="Decode_gn_2"),
            Activation("relu", name="Decode_relu_2"),

            UpSampling2D(size=(2,2), interpolation='bilinear', name="Decode_upsample_3"),
            Conv2D(256, kernel_size=(3,3), strides=(1,1), padding='same', name="Decode_conv_3"),
            GroupNormalization(groups=32, name="Decode_gn_3"),
            Activation("relu", name="Decode_relu_3"),

            UpSampling2D(size=(2,2), interpolation='bilinear', name="Decode_upsample_4"),
            Conv2D(256, kernel_size=(3,3), strides=(1,1), padding='same', name="Decode_conv_4"),
            GroupNormalization(groups=32, name="Decode_gn_4"),
            Activation("relu", name="Decode_relu_4"),

            Conv2D(self.num_classes, kernel_size=(1,1), strides=(1,1), padding='same', 
                dtype='float32', name="Decode_output")  
        ])
        
    
    
    def get_aux_head(self, n):
        aux_head = tf.keras.Sequential([
            Lambda(lambda x: x[:, 1:], name="Aux_{}_ExtractToken".format(n)),
            LayerNormalization(epsilon=1e-6, name="Aux_{}_norm".format(n)),
            Reshape(target_shape=(int(self.image_size//16), int(self.image_size//16), self.hidden_size)),
            Conv2D(
                self.hidden_size, 
                kernel_size=(3,3), 
                strides=(1,1), 
                padding='same', 
                name="Aux_{}_conv_1".format(n)
            ),
            GroupNormalization(groups=32, name="Aux_{}_gn".format(n), axis=-1),
            Activation("relu", name="Aux_{}_relu".format(n)),
            UpSampling2D(size=(4,4), interpolation='bilinear', name="Aux_{}_upsample_1".format(n)),
            Conv2D(
                filters=256, 
                kernel_size=(1,1), 
                strides=(1,1), 
                padding='same', 
                name="Aux_{}_conv_2".format(n)
            ),
            UpSampling2D(size=(4,4), interpolation='bilinear', name="Aux_{}_upsample_2".format(n)),
            Conv2D(
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if self.aux_layers is not None:
            y_aux_list = []
            for i in range(0, len(y_list)):
                y_aux_list.append(self.aux_heads[i](y_list[i]))

            y_aux_list = list(y_aux_list)
            
            
        y = self.extract_token(y)
...
def aux_head(x, n, image_size, hidden_size, num_classes):
    ...
    y_aux = GroupNormalization(groups=32, name="Aux_{}_gn".format(n), axis=-1)(y_aux)
    y_aux = Activation("relu", name="Aux_{}_relu".format(n))(y_aux)
    y_aux = UpSampling2D(size=(4,4), interpolation='bilinear', name="Aux_{}_upsample_1".format(n))(y_aux)
    y_aux = Conv2D(256, kernel_size=(1,1), strides=(1,1), padding='same', name="Aux_{}_conv_2".format(n))(y_aux)
    y_aux = UpSampling2D(size=(4,4), interpolation='bilinear', name="Aux_{}_upsample_2".format(n))(y_aux)
...
def get_decode_head(y, image_size, hidden_size, num_classes):
    ...
    y = GroupNormalization(groups=32, name="Decode_gn_1")(y)
    ...
    y = GroupNormalization(groups=32, name="Decode_gn_2")(y)
    ...
    y = GroupNormalization(groups=32, name="Decode_gn_3")(y)
    ...
    y = GroupNormalization(groups=32, name="Decode_gn_4")(y)
