...
def stem_net(x_input, out_filters):
    x = Conv2D(64, 3, strides=(2, 2), padding='same', use_bias=False)(x_input)
    x = BatchNormalization(axis=3, momentum=BN_MOMENTUM)(x)
    x = Conv2D(64, 3, strides=(2, 2), padding='same', use_bias=False)(x)
    x = BatchNormalization(axis=3, momentum=BN_MOMENTUM)(x)
    x = Activation('relu')(x)

    x = bottleneck_Block(x, out_filters, with_conv_shortcut=True)
    x = bottleneck_Block(x, out_filters, with_conv_shortcut=False)
    x = bottleneck_Block(x, out_filters, with_conv_shortcut=False)
    x = bottleneck_Block(x, out_filters, with_conv_shortcut=False)
...
def clf_head(x, out_filters_list=[32, 64, 128, 256]):
    """
    1) Increase the # channels from C, 2C, 4C, 8C to 128, 256, 512, 1024
...
def HRNet(input_height, input_width, n_classes=20, W=32, channel=3, layername='model'):
    ...
    x = stem_net(inputs, out_filters=C) # (64, 64, 256) x 4
