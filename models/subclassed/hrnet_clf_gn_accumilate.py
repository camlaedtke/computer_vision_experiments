...
from tensorflow.keras.layers import (Conv2D, BatchNormalization, ReLU, UpSampling2D, MaxPool2D,
...
class Bottleneck(tf.keras.layers.Layer):
    ...
    def __init__(self, _in_channel, output_dim, stride=1, downsample=None, GN_GROUPS=32, after_stem=False, **kwargs):
        ...
        self.gn3 = GroupNormalization(groups=GN_GROUPS)
        self.relu = ReLU()
...
class HighResolutionModule(tf.keras.layers.Layer):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels, num_channels, 
                 GN_GROUPS, multi_scale_output=True, **kwargs):
    ...
    def _make_one_branch(self, branch_index, block, num_blocks, num_channels, stride=1):
        ...
        for i in range(1, num_blocks[branch_index]):
            layers.append(block(self.num_inchannels[branch_index], num_channels[branch_index], GN_GROUPS=self.GN_GROUPS))
...
class HRNet_CLF_GN(tf.keras.models.Model):
    def __init__(self, 
                 stage1_cfg, 
                 stage2_cfg, 
                 stage3_cfg, 
        ...
        super(HRNet_CLF_GN, self).__init__()
        ...
        self.layer1 = self._make_layer(block, num_channels, num_blocks, after_stem=True)
        ...
        # Output downsampling layer
        self.maxpool4x4 =  MaxPool2D(pool_size=(2,2))

        # Classification Head
        self.incre_modules, self.downsamp_modules, self.final_layer = self._make_head(pre_stage_channels, GN_GROUPS=32)
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        for i in range(len(y_list)):
            y_list[i] = self.maxpool4x4(y_list[i])
    ...
    def build_model(self):
        ...
        self._name = "HRNet_CLF_W{}_ACCUMILATED".format(self.W)
